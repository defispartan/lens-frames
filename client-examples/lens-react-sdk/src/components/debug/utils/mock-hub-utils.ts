...
import { sortedSearchParamsString } from "@/components/debug/lib/utils";
...
export async function loadMockResponseForDebugHubRequest(
  ...
  const pathAndQuery = `/${hubPath.join("/")}?${sortedSearchParamsString(
    searchParams
  )}`;
  ...
  if (mockResult?.ok != null) {
    console.log(
      `info: Mock hub: Found mock for ${pathAndQuery}, returning ${
        mockResult.ok ? "200" : "404"
      }`
    );
